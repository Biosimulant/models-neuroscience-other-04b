# 261483

**Source**: [opensourcebrain](https://github.com/OpenSourceBrain/261483)
**Standard**: other
**Authors**: Open Source Brain

## Description

Coding explains development of binocular vision and its failure in Amblyopia (Eckmann et al 2020)


## Usage

This model was auto-generated from the opensourcebrain repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/neuroscience-other-coding-explains-development-binocular-vision-its-failure-amblyopia-osb261483-model/model.yaml
```

## Tags

other, opensourcebrain, auto-generated, neuroscience
